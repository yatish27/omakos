...
function josh_prompt {
  ...
  # prompt="%{%F{red}%}[$(git_current_branch)]%{$reset_color%} %{%F{green}%}$PWD$prompt%{%F{red}%}$(ruby_prompt_info)%{$reset_color%}"
  if [[ -n $(git_current_branch) ]]; then
    prompt="%{%F{green}%}$PWD%{$reset_color%} %{%F{red}%}[$(git_current_branch)]%{$reset_color%}$prompt%{%F{red}%}$(ruby_prompt_info)%{$reset_color%}"
  else
    prompt="%{%F{green}%}$PWD$prompt%{%F{red}%}$(ruby_prompt_info)%{$reset_color%}"
  fi
